FROM docker.io/library/debian:12-slim

RUN apt-get update && \
    apt-get install -y --no-install-recommends wget ca-certificates && \
    apt-get clean all

RUN useradd -m cla

WORKDIR /home/cla

USER cla

RUN wget -nv https://github.com/astral-sh/uv/releases/download/0.8.0/uv-x86_64-unknown-linux-gnu.tar.gz && \
    mkdir -p .local/bin/ && \
    tar -xzvf uv-x86_64-unknown-linux-gnu.tar.gz -C .local/bin/ --strip-components=1 && \
    rm uv-x86_64-unknown-linux-gnu.tar.gz .wget-hsts

ENV PATH="$PATH:/home/cla/.local/bin"

COPY base/ ./base/
COPY cla/ ./cla/
COPY pyproject.toml uv.lock manage.py cla-app.sh ./

RUN uv sync --locked --no-dev

# Required environment variables
ENV DJANGO_DEBUG=false \
    DJANGO_ADMIN_SITE_HEADER \
    DJANGO_ADMIN_SITE_INDEX_TITLE \
    DJANGO_ADMIN_SITE_TITLE \
    DJANGO_ALLOWED_HOSTS \
    DJANGO_CCLA_WEBHOOK_SECRET_SLUG \
    DJANGO_CLA_REPLY_TO_EMAIL \
    DJANGO_CLOUDFLARE_TURNSTILE_SECRET_KEY \
    DJANGO_CORS_ALLOW_CREDENTIALS \
    DJANGO_CORS_ALLOWED_ORIGINS \
    DJANGO_CSRF_COOKIE_SECURE \
    DJANGO_CSRF_TRUSTED_ORIGINS \
    DJANGO_DOCUSEAL_CCLA_TEMPALTE_ID \
    DJANGO_DOCUSEAL_ICLA_TEMPLATE_ID \
    DJANGO_DOCUSEAL_KEY \
    DJANGO_EMAIL_BACKEND \
    DJANGO_ICLA_WEBHOOK_SECRET_SLUG \
    DJANGO_NOTIFICATIONS_RECIPIENT_EMAIL \
    DJANGO_NOTIFICATIONS_SENDER_EMAIL \
    DJANGO_SECRET_KEY \
    DJANGO_SESSION_COOKIE_SECURE \
    GUNICORN_CMD_ARGS

CMD ["./cla-app.sh", "migrate-and-run"]
